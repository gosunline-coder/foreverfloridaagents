...
export default function AdminClientLayout({ children }: { children: React.ReactNode }) {
  ...
  // 1. If syncing or not loaded, neutral loading state
  ...
  // 2. If loaded but unauthorized, missing session, or invalid role
  if (isUnauthorized || !isSignedIn || !user || !["admin", "superadmin"].includes(user.role)) {
    return (
      <div className="flex flex-col h-screen items-center justify-center bg-gray-50 p-4 dark:bg-[#0f172a]">
        <div className="bg-white dark:bg-slate-900 p-8 rounded-lg shadow-md max-w-md w-full text-center border border-gray-200 dark:border-slate-800">
          <h2 className="text-2xl font-bold text-red-600 mb-4">Access Denied</h2>
          <p className="text-gray-700 dark:text-gray-300 mb-6">You do not have permission to view the admin portal.</p>
          <button 
            onClick={logout}
