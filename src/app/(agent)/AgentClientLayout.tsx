...
export default function AgentClientLayout({ children }: { children: React.ReactNode }) {
  ...
  if (isUnauthorized || !isSignedIn || !user || !["agent", "admin", "superadmin"].includes(user.role)) {
    return (
      <div className="flex flex-col h-screen items-center justify-center bg-gray-50 p-4">
        <div className="bg-white p-8 rounded-lg shadow-md max-w-md w-full text-center border border-gray-200">
          ...
          <p className="text-gray-700 mb-6">You do not have permission to view this dashboard.</p>
          <button 
            onClick={logout}
